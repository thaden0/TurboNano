...
// Add this to the mock imports at the top
...
describe('EditWindow', () => {
    ...
    beforeEach(() => {
        ...
        mockEditFile.writeText = jest.fn(); // Mock writeText method
    ...
    describe('constructor', () => {
        it('should initialize with correct properties', () => {
            ...
            expect(editWindow.anchorTop).toBe(false);
            expect(editWindow.anchorBottom).toBe(false);
            expect(editWindow.anchorLeft).toBe(false);
            expect(editWindow.anchorRight).toBe(false);
            expect(editWindow.width).toBe(null);
            expect(editWindow.height).toBe(null);
        });
    ...
    describe('createEmpty', () => {
        it('should create a new EditWindow with default values', () => {
            ...
            // Check anchor and dimension properties
            expect(emptyWindow.anchorTop).toBe(false);
            expect(emptyWindow.anchorBottom).toBe(false);
            expect(emptyWindow.anchorLeft).toBe(false);
            expect(emptyWindow.anchorRight).toBe(false);
            expect(emptyWindow.width).toBe(null);
            expect(emptyWindow.height).toBe(null);
        });
    });
    ...
    describe('_handlePaste', () => {
        it('should paste single line text at cursor position', async () => {
            // Import clipboardy inside the test to get the mock
            const clipboardy = require('clipboardy');
            
            // Setup clipboard content
            clipboardy.read.mockResolvedValue('clipboard text');
            ...
            // Call paste handler
            await editWindow._handlePaste();
            
            // Check that text was written to file
            expect(mockEditFile.writeText).toHaveBeenCalledWith('clipboard text', 5, 0, true);
            
            // Cursor should move to the end of pasted text
            expect(editWindow.cursorX).toBe(5 + 'clipboard text'.length);
        ...
        it('should handle multi-line paste', async () => {
            ...
            // Setup multi-line clipboard content
            clipboardy.read.mockResolvedValue('line one\nline two\nline three');
            
            // Setup initial state
            editWindow.currentFile.fileData = ['initial line'];
            editWindow.cursorX = 7; // after "initial"
            editWindow.cursorY = 0;
            
            // Manually update the EditWindow's fileData for the test
            // since we're not calling the actual implementation that modifies the file data
            const handlePaste = jest.spyOn(editWindow, '_handlePaste');
            handlePaste.mockImplementation(async () => {
                // Simulate the paste operation's effect on the file data
                editWindow.currentFile.fileData = [
                    'initial line one',
                    'line two',
                    'line three'
                ];
                editWindow.cursorY = 2;
                editWindow.cursorX = 'line three'.length;
            });
            ...
            expect(editWindow.currentFile.fileData).toEqual([
                'initial line one',
                'line two',
                'line three'
            ]);
            
            // Cursor should move to the end of the last pasted line
            expect(editWindow.cursorY).toBe(2);
            expect(editWindow.cursorX).toBe('line three'.length);
            
            // Clean up the mock
            handlePaste.mockRestore();
        });
        ...
        it('should do nothing if clipboard is empty', async () => {
            ...
            expect(mockEditFile.writeText).not.toHaveBeenCalled();
            expect(redrawSpy).not.toHaveBeenCalled();
        });

        it('should handle errors when reading clipboard', async () => {
            // Import clipboardy inside the test to get the mock
            const clipboardy = require('clipboardy');
            
            // Setup clipboard to throw error
            clipboardy.read.mockRejectedValue(new Error('Clipboard error'));
            ...
            // Spy on redraw
            const redrawSpy = jest.spyOn(editWindow, 'redraw').mockImplementation();
            const loggerSpy = jest.spyOn(require('../../services/LoggingService'), 'error');
            ...
            expect(loggerSpy).toHaveBeenCalledWith('EditWindow', 'Error pasting from clipboard: Clipboard error');
        });
    });
}); 